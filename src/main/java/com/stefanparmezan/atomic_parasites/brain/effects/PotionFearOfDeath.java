...
public class PotionFearOfDeath extends Potion {
    ...
    private static final float CAMERA_SHAKE_INTENSITY = 0.03f;
    private static final float SANITY_DRAIN_PER_TICK = 0.1f;
    private static final double SPEED_BOOST_MULTIPLIER = 0.4;
    ...
    public void performEffect(net.minecraft.entity.EntityLivingBase entity, int amplifier) {
        ...
        float newSanity = Math.max(0.0f, currentSanity - SANITY_DRAIN_PER_TICK);
        BrainManager.setSanity(player, newSanity);

        // 2. Звук (с логом)
        if (player.ticksExisted % SOUND_INTERVAL == 0) {
            AtomicParasites.LOGGER.info("[PotionFearOfDeath] 🔊 Playing sound | Volume: {} | Pitch: {}",
                    0.5f + player.getRNG().nextFloat() * 0.2f,
                    0.85f + player.getRNG().nextFloat() * 0.3f);

            float pitch = 0.85f + player.getRNG().nextFloat() * 0.3f;
            float volume = 0.5f + player.getRNG().nextFloat() * 0.2f;
            player.playSound(AtomicParasites.FEAR_GASP, volume, pitch);
        }

        // 3. Тряска
        if (player.ticksExisted % 4 == 0) {
            AtomicParasites.network.sendTo(new CameraShakePacket(CAMERA_SHAKE_INTENSITY), player);
        }

        // 4. Скорость
        applyHiddenSpeed(player);

        // 5. Слепота (с защитой от мерцания)
        boolean hasBlindness = player.isPotionActive(MobEffects.BLINDNESS);
        if (!hasBlindness || player.getActivePotionEffect(MobEffects.BLINDNESS).getDuration() < 50) {
            player.addPotionEffect(new PotionEffect(MobEffects.BLINDNESS, 200, 0, false, false));
            AtomicParasites.LOGGER.debug("[PotionFearOfDeath] 👁️ Blindness refreshed");
        }
    }
